...
class EBRDatabaseManager:
    ...
    def create_tables(self):
        ...
        # Removed branch1_irreps, branch2_irreps
        # Added updated_at
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS ebrs (
          id             INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # New table for decomposition branches
        cursor.execute("""
        ...
        AFTER UPDATE ON space_groups
        FOR EACH ROW
        BEGIN
            UPDATE space_groups SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id;
        END;
        """)
        cursor.execute("""
        CREATE TRIGGER IF NOT EXISTS update_ebrs_updated_at
        AFTER UPDATE ON ebrs
        FOR EACH ROW
        BEGIN
            UPDATE ebrs SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id;
        END;
        """)
        
        self.conn.commit()

    # --- Methods for parsing and validation (parse_wyckoff, parse_orbital, etc.) remain largely the same ---
    def parse_wyckoff(self, entry):
        m = re.match(r"^(\d+\w)\(([^)]+)\)$", entry)
    ...
    def validate_input_file(self, filepath):
        ...
        try:
            ...
            required = ["Wyckoff pos.", "Band-Rep.", "Decomposable"]
            missing = [s for s in required if s not in content]
            if missing: return False, f"Missing required sections: {', '.join(missing)}"
            return True, "File format appears valid."
        except Exception as e: return False, f"Error reading file: {str(e)}"

    # --- Ingest logic (_parse_file_content, _parse_raw_text, ingest_file, ingest_text) ---
    # These methods will need minor adjustments if they previously relied on branch1/2_irreps in ebrs.
    # _insert_data will no longer insert into ebrs.branch1/2_irreps.
    def _parse_file_content(self, raw_lines):
        joined = "\n".join(raw_lines)
        joined = re.sub(r'\\\s*\n\s*', ' ', joined)
        lines = [line.rstrip("\n") for line in joined.splitlines() if line.strip()]
    ...
    def ingest_file(self, sg_number, filepath):
        ...
        with open(filepath, 'r', encoding='utf-8') as f: raw_lines = [l.rstrip("\n") for l in f if l.strip()]
        
        wyckoff_line, orbital_line, notes_line, kpoint_lines = self._parse_file_content(raw_lines)
        wyckoff_entries, orbital_entries, notes_entries = wyckoff_line.split(), orbital_line.split(), notes_line.split()
        num_cols = len(wyckoff_entries)

        if len(notes_entries) > num_cols: notes_entries = notes_entries[-num_cols:]
        elif len(notes_entries) < num_cols and not (len(notes_entries) == 0 and num_cols >0):
            raise ValueError(f"Col count mismatch: Wyckoff({num_cols}), Orbital({len(orbital_entries)}), Notes({len(notes_entries)})")
        if not (len(orbital_entries) == num_cols and (len(notes_entries) == num_cols or (len(notes_entries) == 0 and num_cols >0))):
            raise ValueError(f"Col count mismatch after trim: Wyckoff({num_cols}), Orbital({len(orbital_entries)}), Notes({len(notes_entries)})")
        
        kpoint_data = []
        for kp_line in kpoint_lines:
            tokens = kp_line.split()
            if tokens: kpoint_data.append((tokens[0].rstrip(":"), tokens[1:]))
        return self._insert_data(sg_number, wyckoff_entries, orbital_entries, notes_entries, kpoint_data)
    ...
    def ingest_text(self, sg_number, raw_text):
        ...
        wyckoff_line, orbital_line, notes_line, kpoint_data_parsed = self._parse_raw_text(raw_text) # kpoint_data_parsed is list of (label, cells)
        wyckoff_entries, orbital_entries, notes_entries = wyckoff_line.split(), orbital_line.split(), notes_line.split()
        num_cols = len(wyckoff_entries)

        if len(notes_entries) > num_cols: notes_entries = notes_entries[-num_cols:]
        elif len(notes_entries) < num_cols and not (len(notes_entries) == 0 and num_cols >0):
             raise ValueError(f"Col count mismatch: W({num_cols}),O({len(orbital_entries)}),N({len(notes_entries)})")
        if not (len(orbital_entries) == num_cols and (len(notes_entries) == num_cols or (len(notes_entries) == 0 and num_cols >0))):
            raise ValueError(f"Col count mismatch post-trim: W({num_cols}),O({len(orbital_entries)}),N({len(notes_entries)})")
        return self._insert_data(sg_number, wyckoff_entries, orbital_entries, notes_entries, kpoint_data_parsed)


    def _insert_data(self, sg_number, wyckoff_entries, orbital_entries, notes_entries_list, kpoint_data_list_of_tuples):
        cursor = self.conn.cursor()
        cursor.execute("INSERT OR REPLACE INTO space_groups(number, symbol, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)", (sg_number, None)) # Symbol not parsed yet
        sg_id = cursor.lastrowid if cursor.lastrowid else cursor.execute("SELECT id FROM space_groups WHERE number = ?", (sg_number,)).fetchone()[0]
        
        # Delete old EBRs and their dependent irreps/branches (CASCADE will handle dependents)
        cursor.execute("DELETE FROM ebrs WHERE space_group_id = ?", (sg_id,))
        self.conn.commit()
        ...
        if num_cols > 0 and len(notes_final) != num_cols:
            if not notes_final: notes_final = ["indecomposable"] * num_cols
            elif len(notes_final) == 1 and num_cols > 1: notes_final = [notes_final[0]] * num_cols
            else: raise ValueError(f"Notes entries ({len(notes_final)}) vs cols ({num_cols}) mismatch.")
        ...
        for j in range(num_cols):
            ...
            note_j_status = notes_final[j].lower() if j < len(notes_final) else "indecomposable"
            ...
            for (kp_label, cells_for_kpoint) in kpoint_data_list_of_tuples:
                ...
                else: raise ValueError(f"Mismatch irrep data for k-pt {kp_label}, col {j}")
        self.conn.commit()
        return num_cols
    ...
    def add_ebr_decomposition_branch(self, ebr_id, decomposition_index, branch1_str, branch2_str):
        """Adds or replaces a specific decomposition branch pair for an EBR."""
        cursor = self.conn.cursor()
        try:
            cursor.execute("""
            ...
            self.conn.commit()
            # Update the parent EBR's updated_at timestamp
            cursor.execute("UPDATE ebrs SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", (ebr_id,))
            self.conn.commit()
            return True
        except sqlite3.IntegrityError as e:
            # This could happen if ebr_id doesn't exist, though foreign key should catch it.
            # Or other unique constraint violations if any were added.
            print(f"Error adding decomposition branch: {e}")
            return False

    def get_ebr_decomposition_branches(self, ebr_id):
        """Retrieves all decomposition branches for a given EBR ID."""
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT decomposition_index, branch1_irreps, branch2_irreps, created_at
            FROM ebr_decomposition_branches
            WHERE ebr_id = ?
            ORDER BY decomposition_index
        """, (ebr_id,))
        return cursor.fetchall()

    def delete_ebr_decomposition_branch(self, ebr_id, decomposition_index):
        """Deletes a specific decomposition branch."""
        cursor = self.conn.cursor()
        cursor.execute("""
            DELETE FROM ebr_decomposition_branches
            WHERE ebr_id = ? AND decomposition_index = ?
        """, (ebr_id, decomposition_index))
    ...
    def delete_all_ebr_decomposition_branches(self, ebr_id):
        """Deletes all decomposition branches for a specific EBR ID."""
        cursor = self.conn.cursor()
...
def interactive_mode():
    ...
    while True:
        ...
        print("2. Input raw data for specific space group")
        print("3. Input from file for specific space group")
        print("4. Manage decomposition branches for an EBR") # NEW OPTION
        ...
        try: # Wrap choices in try-except for cleaner error handling
            if choice == "1":
                if next_sg is None: print("ℹ️ No 'next' SG. Use option 2."); continue
                sg_to_process = next_sg
                print(f"\nEntering data for SG {sg_to_process}. Paste raw data (Ctrl+D or 2 empty lines to end):")
                print("-" * 80)
                raw_lines = []
                empty_c = 0
                while True:
                    try: line = input(); raw_lines.append(line)
                    except EOFError: break
                    if not line.strip(): empty_c+=1
                    else: empty_c=0
                    if empty_c >=2: break
                raw_data = "\n".join(raw_lines)
                if raw_data.strip():
                ...
                print(f"\nEntering data for SG {sg_num}. Paste raw data (Ctrl+D or 2 empty lines to end):")
                print("-" * 80)
                raw_lines = []
                empty_c = 0
                while True:
            ...
            elif choice == "4": # Manage decomposition branches
                ...
                ebr_map = {}
                for i, ebr in enumerate(decomposable_ebrs):
                    ebr_map[str(i+1)] = ebr[0] # Map display index to ebr_id
                    print(f"  {i+1}. EBR ID: {ebr[0]}, Wyckoff: {ebr[1]}{ebr[2]}, Orbital: {ebr[3]}")
                
                ebr_choice_idx = input("Select EBR by number to manage branches: ").strip()
                selected_ebr_id = ebr_map.get(ebr_choice_idx)
                if not selected_ebr_id:
                    print("❌ Invalid selection.")
                    continue

                print(f"\n--- Managing Branches for EBR ID: {selected_ebr_id} ---")
                existing_branches = db.get_ebr_decomposition_branches(selected_ebr_id)
                ...
                while True:
                    branch_action = input("\nBranch actions: (A)dd new, (D)elete specific, (C)lear all for this EBR, (R)eturn to main menu: ").strip().upper()
                    if branch_action == 'A':
                        try:
                            dec_idx = int(input("  Enter decomposition index (e.g., 1, 2): ").strip())
                            b1_str = input(f"  Enter Branch 1 irreps string for index {dec_idx}: ").strip()
                            b2_str = input(f"  Enter Branch 2 irreps string for index {dec_idx}: ").strip()
                            if not b1_str or not b2_str: print("❌ Branch strings cannot be empty."); continue
                            if db.add_ebr_decomposition_branch(selected_ebr_id, dec_idx, b1_str, b2_str):
                                print(f"✅ Branch index {dec_idx} added/updated for EBR ID {selected_ebr_id}.")
                            else: print(f"❌ Failed to add branch index {dec_idx}.")
                        except ValueError: print("❌ Invalid index number.")
                    elif branch_action == 'D':
                        try:
                            dec_idx_del = int(input("  Enter decomposition index to delete: ").strip())
                            if db.delete_ebr_decomposition_branch(selected_ebr_id, dec_idx_del):
                                print(f"✅ Branch index {dec_idx_del} deleted for EBR ID {selected_ebr_id}.")
                            else: print(f"ℹ️ Branch index {dec_idx_del} not found or could not be deleted.");
                        except ValueError: print("❌ Invalid index number.")
                    elif branch_action == 'C':
                        confirm_clear = input(f"⚠️ Are you sure you want to delete ALL branches for EBR ID {selected_ebr_id}? (yes/no): ").strip().lower()
                        if confirm_clear == 'yes':
                            count = db.delete_all_ebr_decomposition_branches(selected_ebr_id)
                            print(f"✅ Deleted {count} branches for EBR ID {selected_ebr_id}.")
                        else: print("ℹ️ Clear operation cancelled.")
                    elif branch_action == 'R': break
                    else: print("❌ Invalid branch action.")
                    
                    # Refresh and show branches after action
                    current_branches_after_action = db.get_ebr_decomposition_branches(selected_ebr_id)
                    if current_branches_after_action:
                        print("  Current Branches:")
                        for br_idx_curr, b1_curr, b2_curr, ca_curr in current_branches_after_action:
                            print(f"    Index {br_idx_curr}: Branch1='{b1_curr}', Branch2='{b2_curr}' (Added: {ca_curr})")
                    else:
                        print("  No existing branches for this EBR after action.")
                ...
                if status['missing_sgs']:
                    ...
                    print(f"  Missing SGs: {mc} total. First 10: {', '.join(status['missing_sgs'][:10])}" if mc > 10 else f"  Missing SGs: {', '.join(status['missing_sgs'])}")
                else: print("  ✅ All 230 SGs present.")
                ...
                if result:
                    ...
                    for i, ebr_entry in enumerate(result['ebrs'], 1):
                        ...
                        if ebr_entry['branches']:
                            ...
                            for dec_idx, b1, b2, ca_br in ebr_entry['branches']:
...
def main():
    parser = argparse.ArgumentParser(
        description="EBR Database Manager with Decomposition Branch Support",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  %(prog)s --interactive
  %(prog)s --sg 12 data_sg12.txt
  %(prog)s --status
  %(prog)s --query --sg 12"""
    )
    ...
    try:
        if args.interactive: interactive_mode()
            ...
            # (Simplified CLI query output for brevity, interactive mode has more detail)
            if res: print(f"Data found for SG {args.sg}. EBRs: {len(res['ebrs'])}")
            ...
            db_manager_cli = EBRDatabaseManager()
            num = db_manager_cli.ingest_file(args.sg, args.input_file)
            print(f"✅ Imported {num} EBRs for SG {args.sg} from '{args.input_file}'")
